...
class CreateCustomer implements Tool
{
    ...
    public function schema(JsonSchema $schema): array
    {
        return [
            ...
            'addresses' => $schema
                ->array()
                ->description('Lista de direcciones del cliente (máx. 20). Cada elemento puede tener country_name, state_name y address. Envía null si no aplica.')
                ->max(20)
                ->nullable()
                ->required()
                ->items($schema->object([
                    'country_name' => $schema->string()->description('País.')->max(120)->nullable()->required(),
                    'state_name' => $schema->string()->description('Estado o provincia.')->max(120)->nullable()->required(),
                    'address' => $schema->string()->description('Dirección textual completa.')->max(2000)->nullable()->required(),
                ])),
        ];
    }
}
